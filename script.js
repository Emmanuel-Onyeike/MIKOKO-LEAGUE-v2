...
function showAlert(title, message) {
    ...
    if ("Notification" in window && Notification.permission === "granted") {
        new Notification(title, {
            body: message,
            icon: "/icon-192.png",  // relative path - make sure file exists in root
            tag: "mikoko-alert",
...
// ────────────────────────────────────────────────
// REAL-TIME LISTENERS FOR ALL CATEGORIES
// ────────────────────────────────────────────────
...
window.onload = () => {
    ...
    // Initial news snapshot (fallback)
    if (typeof db !== 'undefined') {
